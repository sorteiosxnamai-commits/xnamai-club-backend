...
function hasJoined(user: User) {
  return (user.subscriptions ?? []).some((subscription) => (
    JOINED_STATUSES.has(subscription.status) || Boolean(subscription.startedAt)
  ));
}
...
atendimentoRouter.get('/members', async (_req, res) => {
  ...
  res.json(users.filter(hasJoined).map(serializeMember));
});
...
atendimentoRouter.post('/members/:id/cashback-use', async (req, res) => {
  ...
  if (!user || !hasJoined(user)) {
    return res.status(404).json({ message: 'Cliente do clube n?o encontrado.' });
  }

  const member = serializeMember(user);
  if (!member.cashback.eligible) {
    return res.status(400).json({ message: 'Este cliente n?o tem cashback de lan?amento.' });
  }
  if (user.launchCashbackUsedAt) {
    return res.status(409).json({ message: 'Cashback j? utilizado.', member: serializeMember(user) });
  }
